...
const STANCE_STYLE: Record<string, string> = {
  agree: "bg-[var(--color-good-soft)] text-[var(--color-good)]",
  adjust: "bg-[var(--color-warn-soft)] text-[var(--color-warn)]",
  disagree: "bg-[var(--color-bad-soft)] text-[var(--color-bad)]",
};
    ...
    <div className="space-y-3">
      <form onSubmit={invite} className="flex gap-2">
        <input
          ...
          required
          className="flex-1 text-sm border border-[var(--color-border)] rounded-md px-3 py-2 focus:outline-none focus:border-[var(--color-accent)]"
        />
        <button
          type="submit"
          disabled={inviting}
          className="text-sm bg-[var(--color-accent)] text-white rounded-md px-4 hover:bg-[var(--color-accent-dark)] disabled:opacity-50"
        >
          Invite to review
        </button>
      </form>
      ...
      {notes.length > 0 && (
        <div className="space-y-2">
          {notes.map((n) => (
            <div key={n.note_id} className="border border-[var(--color-court)]/40 bg-[var(--color-court-soft)] rounded-lg p-3">
              <div className="flex items-center gap-2 mb-1 text-xs">
                <span className="font-medium">{n.coach_name}</span>
                <button onClick={() => onSeek(n.timestamp_s)} className="text-[var(--color-accent)] hover:underline">

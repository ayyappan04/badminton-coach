  ...
  return (
    <div className="space-y-4">
      <div className="grid sm:grid-cols-3 gap-3">
        {fs && (
          <div className="border border-[var(--color-border)] rounded-lg p-3 bg-[var(--color-bg-raised)]">
            <div className="flex justify-between items-baseline mb-2">
              <h3 className="text-sm font-medium">Formation split</h3>
              <span className="text-[10px] text-[var(--color-ink-soft)]">{Math.round(block.confidence * 100)}% conf.</span>
            </div>
            <div className="flex h-2.5 rounded-full overflow-hidden mb-1.5">
              <div className="bg-[var(--color-accent)]" style={{ width: `${fs.front_back_pct}%` }} />
              <div className="bg-[var(--color-court)]" style={{ width: `${fs.side_by_side_pct}%` }} />
            </div>
            <p className="text-xs text-[var(--color-ink-soft)]">
              <span className="text-[var(--color-accent)]">■</span> Front-back {fs.front_back_pct}% ·{" "}
              <span className="text-[var(--color-court)]">■</span> Side-by-side {fs.side_by_side_pct}%
            </p>
          </div>
        )}
          ...
          <div className="border border-[var(--color-border)] rounded-lg p-3 bg-[var(--color-bg-raised)]">
            ...
            <p className="text-xs text-[var(--color-ink-soft)] leading-relaxed">
              ...
              {rot.missed_rotations > 0 && (
                <> <span className="text-[var(--color-warn)]">{rot.missed_rotations}</span> never reached front-back.</>
              )}
              {rot.avg_rotation_delay_s !== null && <> Average rotation took <span className="text-[var(--color-ink)]">{rot.avg_rotation_delay_s}s</span>.</>}
            </p>
          </div>
        )}
          ...
          <div className="border border-[var(--color-border)] rounded-lg p-3 bg-[var(--color-bg-raised)]">
            ...
            <p className="text-xs text-[var(--color-ink-soft)] leading-relaxed">
              ...
              {sp.open_middle_in_defense_pct !== null && (
                <> Middle channel open during defense <span className={sp.open_middle_in_defense_pct > 30 ? "text-[var(--color-warn)]" : ""}>{sp.open_middle_in_defense_pct}%</span>.</>
              )}
            </p>
          </div>
        )}
      ...
      {block.findings && block.findings.length > 0 && (
        <div className="space-y-2">
          {block.findings.map((f, i) => (
            <div key={i} className="border-l-2 border-[var(--color-accent)] pl-3 py-1">
              <p className="text-sm">{f.finding}</p>
              <p className="text-xs text-[var(--color-ink-soft)] mt-0.5">Try: {f.suggestion}</p>
            </div>
          ))}
        </div>
      )}

      <p className="text-[11px] text-[var(--color-ink-soft)]">{block.basis}</p>
    </div>
  );
}

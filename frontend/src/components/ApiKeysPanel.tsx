    ...
    <div className="space-y-3">
      ...
      <form onSubmit={create} className="flex gap-2">
        <input
          ...
          className="flex-1 text-xs border border-[var(--separator)] rounded-md px-2.5 py-1.5 focus:outline-none focus:border-[var(--accent)]"
          required
        />
      ...
      {freshKey && (
        <div className="border border-[var(--warning)]/40 bg-[var(--warning-soft)] rounded-lg p-3">
      ...
      {keys.map((k) => (
        <div key={k.key_id} className="border border-[var(--separator)] rounded-lg bg-[var(--surface)] p-3 flex items-center justify-between gap-3">
          <div className="min-w-0">
            <p className="text-sm truncate">{k.name} <code className="text-xs text-[var(--text-secondary)]">{k.key_prefix}…</code></p>
            <p className="text-xs text-[var(--text-secondary)]">
              {k.scopes}{k.revoked ? " · revoked" : k.last_used_at ? ` · last used ${k.last_used_at.slice(0, 10)}` : " · never used"}
            </p>
          </div>
          {!k.revoked && (
            <button onClick={() => revoke(k.key_id)} className="text-xs border border-[var(--negative)]/50 text-[var(--negative)] rounded-md px-3 py-1.5 shrink-0 hover:bg-[var(--negative-soft)]">
              Revoke
            </button>
          )}
        </div>
